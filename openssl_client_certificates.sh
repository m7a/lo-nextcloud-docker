...
# and  https://stackoverflow.com/questions/45628601/client-authentication-using-self-signed-ssl-certificate-for-nginx

. .env # source VIRTUAL_HOST variable into script

# Generate CA certificate
openssl genrsa -out ca.key 4096
openssl req -new -x509 -days 8000 -sha256 -subj "/C=DE/ST=Hesse/L=/O=root/OU=/CN=$VIRTUAL_HOST" -key ca.key -out ca.crt

for clnt in 01 02 03 04 05; do
	# Create the Client Key and CSR
	openssl genrsa -out client$clnt.key 4096
	openssl req -new -key client$clnt.key -subj "/C=DE/ST=Hesse/L=/O=clnt$clnt/OU=/CN=$VIRTUAL_HOST" -sha256 -out client$clnt.csr
